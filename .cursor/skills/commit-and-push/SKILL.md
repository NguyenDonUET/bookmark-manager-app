...
description: Review code changes, group them into logical commits, write clear commit messages, and push to the remote repository. Use whenever the user asks to "commit", "commit and push", "save changes to git", or similar.
---

# Commit & Push Skill

## Goal
Take the current working-tree changes, split them into **small, logically coherent commits**, write clear commit messages, and push to the remote branch.

## Step 1 — Inspect the changes
1. Run `git status` to see all modified / added / deleted / untracked files.
2. Run `git diff` (and `git diff --staged` if anything is already staged) to understand *what* changed, not just *which files*.
